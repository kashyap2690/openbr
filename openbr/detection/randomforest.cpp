...
void Detection::RandomForest::Train(
                         ...
                         )
{
    ...
    for (int i = 0; i < num_landmark_; i++) {
        ...
        for (int j = 0; j < max_numtrees_; j++) {
            ...
            for (int k = is; k <= ie; k++)
                index.push_back(k);
            rfs_[i][j].Train(images, ground_truth_shapes, current_shapes, bounding_boxs, mean_shape, shapes_residual, index, i, max_numfeats, max_radio_radius);
        }
...
void Detection::RandomForest::Write(std::ofstream &fout)
{
...
void Detection::RandomForest::Read(std::ifstream &fin)
{
    fin >> max_numtrees_;
    fin >> num_landmark_;
    fin >> max_depth_;
