...
void Detection::Tree::Train(const vector<Mat_<uchar> > &images,
                           ...
                           )
{
    ...
    max_numfeats_ = max_numfeats;
    max_radio_radius_ = max_radio_radius;
    num_nodes_ = 1;
    num_leafnodes_ = 1;
        ...
        for (int n = 0; n < num_nodes_iter; n++) {
            if (!nodes_[n].issplit) {
                if (nodes_[n].depth == max_depth_) {
                    ...
                    Splitnode(images, ground_truth_shapes, current_shapes, bounding_boxs, mean_shape, shapes_residual, nodes_[n].ind_samples, thresh, feat, isvaild, lcind, rcind);
...
void Detection::Tree::Splitnode(const vector<Mat_<uchar> > &images,
                                const vector<Mat_<double> > &ground_truth_shapes,
                                const vector<Mat_<double> > &current_shapes,
...
//    double var_overall = ind_samples.size()*((E_x_2 - E_x*E_x) + (E_y_2 - E_y*E_y));
    ...
    //qDebug() << "max_id:" << max_id << "max_var_reductions:" << max_var_reductions;
    //qDebug() << "feats:" << feat[0] << feat[1] << feat[2] << feat[3];
...
void Detection::Tree::Write(std::ofstream &fout)
{
...
void Detection::Tree::Read(std::ifstream& fin)
{
    fin >> landmarkID_;
    fin >> max_depth_;
    fin >> max_numnodes_;
